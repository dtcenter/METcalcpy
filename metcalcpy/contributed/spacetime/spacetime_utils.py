...
def lonFlip(data,lon):
    ...
    if lonnew.min() < 0:
        ...
        dataflip = xr.DataArray(np.roll(data, nlonneg, axis=2),
             dims=data.dims,
             coords={'time': data['time'], 'lat': data['lat'],
                     'lon': lonnew})
        ...
        dataflip = xr.DataArray(np.roll(data, nlonpos, axis=2),
            dims=data.dims,
            coords={'time': data['time'],
            'lat': data['lat'], 'lon': lonnew})
