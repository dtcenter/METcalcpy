...
class AggStatEventEqualize:
    ...
    def __init__(self, in_params):
        ...
        logger = self.logger
        safe_log(logger, "debug", "Initializing AggStatEventEqualize with parameters.")
    ...
    def calculate_values(self):
        ...
        if not self.input_data.empty:
            safe_log(logger, "info", "Input data is available. Proceeding with event equalization.")
            # list all fixed variables
            if 'fixed_vars_vals_input' in self.params:
                safe_log(logger, "info", "Fixed variables detected. Preparing permutations of fixed values.")
                ...
                for key in self.params['fixed_vars_vals_input']:
                    ...
                    safe_log(logger, "debug", f"Fixed values for {key}: {vals_permuted}")

                fix_vals_permuted = [item for sublist in fix_vals_permuted_list for item in sublist]
                safe_log(logger, "debug", f"All fixed value permutations: {fix_vals_permuted}")
            else:
                fix_vals_permuted = []
                safe_log(logger, "info", "No fixed variables provided for event equalization.")
            # perform EE for each forecast variable on y1 axis
            safe_log(logger, "info", "Running event equalization on axis 1.")
            output_ee_data = self.run_ee_on_axis(fix_vals_permuted, '1')
            safe_log(logger, "debug", f"Event equalization for axis 1 completed. Number of records: {len(output_ee_data)}")
            ...
            if self.params['series_val_2']:
                safe_log(logger, "info", "Series values for axis 2 detected. Running event equalization on axis 2.")
                output_ee_data_2 = self.run_ee_on_axis(fix_vals_permuted, '2')
                safe_log(logger, "debug", f"Event equalization for axis 2 completed. Number of records: {len(output_ee_data_2)}")
                output_ee_data = output_ee_data.drop('equalize', axis=1)
                output_ee_data_2 = output_ee_data_2.drop('equalize', axis=1)
                safe_log(logger, "info", "Concatenating event equalized data from axis 1 and axis 2.")
                ...
                for key in self.params['series_val_2']:
                    ...
                    safe_log(logger, "debug", f"Series variable values for {key}: {all_series_vars[key]}")
                safe_log(logger, "info", "Running event equalization on all data.")
                ...
                output_ee_data = event_equalize(all_ee_records, self.params['indy_var'],
                                                ...
                                                False, logger=logger)
                safe_log(logger, "debug", f"Final event equalization completed. Number of records: {len(output_ee_data)}")

        else:
            output_ee_data = pd.DataFrame()
            safe_log(logger, "warning", "Input data is empty. No event equalization will be performed.")
    ...
    def run_ee_on_axis(self, fix_vals_permuted, axis='1'):
       ...
       """
        logger = self.logger
        safe_log(logger, "debug", f"Running event equalization for axis {axis}.")
        ...
        for series_var, series_var_vals in self.params['series_val_' + axis].items():
            ...
            safe_log(logger, "debug", f"Ungrouped series variable values: {series_var_vals_no_group}")
            ...
            safe_log(logger, "debug", f"Number of records after filtering: {len(series_data_for_ee)}")

            # perform EE on filtered data
            safe_log(logger, "info", f"Performing event equalization on {series_var}.")
            series_data_after_ee = \
                event_equalize(series_data_for_ee, self.params['indy_var'],
                               self.params['series_val_' + axis],
                               list(self.params['fixed_vars_vals_input'].keys()),
                               fix_vals_permuted, True, False, logger=logger)
            safe_log(logger, "debug", f"Number of records after event equalization for {series_var}: {len(series_data_after_ee)}")

            # append EE data to result
            if output_ee_data.empty:
                output_ee_data = series_data_after_ee
                safe_log(logger, "debug", "Initialized output data with the first set of event equalized data.")
            else:
                warnings.simplefilter(action="error", category=FutureWarning)
