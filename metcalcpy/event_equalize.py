...
def event_equalize(series_data, indy_var, series_var_vals, fix_vars, fix_vals_permuted, equalize_by_indep, multi, logger=None):
    ...
    if 'equalize' not  in  series_data.columns:
        ...
        safe_log(logger, "debug", "Equalization column added to the data frame.")
    ...
    if series_var_vals:
        ...
        safe_log(logger, "debug", f"Series variables added for equalization: {vars_for_ee}")
    ...
    if fix_vars:
        ...
        safe_log(logger, "debug", f"Fixed variables added for equalization: {vars_for_ee}")
    ...
    for permutation_index, permutation in enumerate(vars_for_ee_permuted):
        ...
        if multi is False and len(permutation_data['equalize']) \
            ...
            safe_log(logger, "warning", f"Non-unique events detected for permutation {permutation}.")
            print(
        ...
        if permutation_index == 0:
            ...
            for discarded_case in discarded_cases:
                safe_log(logger, "warning", f"Discarding series member with case {discarded_case} for {permutation}")
    ...
    if len(series_data_ee) != len(series_data):
        safe_log(logger, "warning", f"Event equalization removed {len(series_data) - len(series_data_ee)} rows.")
