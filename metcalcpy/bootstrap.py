...
def bootstrap_and_value(values, stat_func, alpha=0.05,
                        ...
                        save_data=True, save_distributions=False, block_length: int = 1, eclv: bool = False, logger=None):
    ...
    distributions = _bootstrap_distribution_cbb(values_lists,
                                                ...
                                                num_threads, block_length, logger=logger)

    bootstrap_dist = do_division(*distributions)
    if eclv:
        result = _get_confidence_interval_and_value_eclv(bootstrap_dist, stat_val, alpha, ci_method, logger=logger)
    else:
        result = _get_confidence_interval_and_value(bootstrap_dist, stat_val, alpha, ci_method,logger=logger)
    if save_data:
...
def _bootstrap_distribution_cbb(values_lists, stat_func_lists,
                                num_iterations, iteration_batch_size, num_threads, block_length=1, logger=None):
    ...
    safe_log(logger, "info", "Starting circular block bootstrap distribution process.")
    safe_log(logger, "debug","Validating input arrays.")
    _validate_arrays(values_lists, logger=logger)
    ...
    safe_log(logger, "debug",f"Iteration batch size set to {iteration_batch_size}.")
    ...
    if num_threads <= 1:
        results = _bootstrap_sim_cbb(values_lists, stat_func_lists,
                                     num_iterations, iteration_batch_size, None, block_length, logger=logger)
    else:
        pool = _multiprocessing.Pool(num_threads)

        iter_per_job = _np.ceil(num_iterations * 1.0 / num_threads)
        safe_log(logger, "debug",f"Iterations per thread: {iter_per_job}.")
...
def bootstrap_and_value_mode(values, cases, stat_func, alpha=0.05,
                             num_iterations=1000, iteration_batch_size=None,
                             num_threads=1, ci_method='perc',
                             save_data=True, save_distributions=False, block_length=1, logger=None):
    ...
    try:
        ...
        safe_log(logger, "debug",f"Selected {len(values_current)} cases for calculation.")
        stat_val = stat_func(values_current)[0]
        safe_log(logger, "info", f"Calculated statistic value: {stat_val}")
        distributions = _bootstrap_distribution_cbb(values_lists,
                                                    stat_func_lists,
                                                    num_iterations,
                                                    iteration_batch_size,
                                                    num_threads, block_length, logger=logger)
        safe_log(logger, "debug",f"Bootstrap distributions: {distributions}")

        bootstrap_dist = do_division(*distributions)
        safe_log(logger, "debug",f"Result after division operation: {bootstrap_dist}")
        result = _get_confidence_interval_and_value(bootstrap_dist, stat_val, alpha, ci_method, logger=logger)
        safe_log(logger, "info", f"Confidence intervals calculated: {result.lower_bound}, {result.upper_bound}")

        if save_data:
            safe_log(logger, "debug","Saving original values to the result.")
            result.set_original_values(values)
        if save_distributions:
...
def _get_confidence_interval_and_value(bootstrap_dist, stat_val, alpha, ci_method, logger=None):
    ...
    if stat_val is None:
        safe_log(logger, "warning", "Statistic value is None. Setting confidence interval method to 'None'.")
        ci_method = "None"

    if ci_method == 'pivotal':
        safe_log(logger, "info", "Using pivotal method to calculate confidence intervals.")
        try:
        ...
        safe_log(logger, "info", "Using percentile method to calculate confidence intervals.")
        ...
        if _all_the_same(bootstrap_dist):
            safe_log(logger, "warning", f"All values of the bootstrap distribution are equal to {bootstrap_dist[0]}. Cannot calculate confidence intervals.")
            ...
            safe_log(logger, "debug",f"Percentile method results: low={low}, stat_val={stat_val}, high={high}")
        val = stat_val
    else:
        safe_log(logger, "warning", "No valid confidence interval method selected.")
        low = None
        val = None
        high = None
    safe_log(logger, "info", f"Finished confidence interval calculation: low={low}, value={val}, high={high}")
    return BootstrapResults(lower_bound=low,
                            value=val,
                            upper_bound=high)


def _get_confidence_interval_and_value_eclv(bootstrap_dist, stat_val, alpha, ci_method, logger=None):
    ...
    # TODO Only percentile method for the confident intervals is implemented
    if stat_val is None:
        safe_log(logger, "warning", "Statistic value is None. Skipping confidence interval calculation.")
        val = None
        low = None
        high = None

    else:
        safe_log(logger, "debug","Preparing bootstrap distribution for confidence interval calculation.")
        bd = bootstrap_dist[bootstrap_dist != _np.array([None])]
        all_values = []
        ...
        safe_log(logger, "debug",f"Calculated steps length: {steps_len}.")
        safe_log(logger, "debug",f"Checking if values contain None: {none_in_values}.")
        ...
        for ind in range(steps_len):
            ...
            if ci_method == 'pivotal':
                ...
                safe_log(logger, "debug",f"Pivotal method result for step {ind}: low={low}, high={high}.")

            elif ci_method == 'perc':
                safe_log(logger, "info", "Using percentile method for confidence interval calculation.")
                if _all_the_same(column):
                    safe_log(logger, "warning", f"All values of column at step {ind} are equal to {column[0]}. Skipping confidence interval calculation.")
    ...
    safe_log(logger, "info", f"Finished confidence interval calculation: value={val}, lower_bounds={low}, upper_bounds={high}.")
    return BootstrapResults(lower_bound=low,
                            value=val,
                            upper_bound=high)
...
def _bootstrap_sim_cbb(values_lists, stat_func_lists, num_iterations,
    ...
    if seed is not None:
        safe_log(logger, "debug",f"Setting random seed: {seed}")
        _np.random.seed(seed)

    num_iterations = int(num_iterations)
    iteration_batch_size = int(iteration_batch_size)
    safe_log(logger, "info", f"Number of iterations: {num_iterations}, iteration batch size: {iteration_batch_size}, block length: {block_length}.")
    ...
    for rng in range(0, num_iterations, iteration_batch_size):
        ...
        safe_log(logger, "debug",f"Running bootstrap iteration batch from {rng} to {rng + max_rng}.")
        try:
        ...
        for i, values_sim, stat_func in zip(range(len(values_sims)), values_sims, stat_func_lists):
            safe_log(logger, "debug",f"Calculating statistic for distribution set {i}.")
            results[i].extend(stat_func(values_sim))
    safe_log(logger, "info", "Completed bootstrap simulation.")
    return _np.array(results)


def _generate_distributions_cbb(values_lists, num_iterations, block_length=1, logger=None):
    ...
    if block_length > 1:
        # uss CBB
        safe_log(logger, "info", f"Applying Circular Block Bootstrap (CBB) with block length: {block_length}")
        ids = _np.apply_along_axis(apply_cbb, axis=1, arr=ids)

    results = [values[ids] for values in values_lists]
    safe_log(logger, "info", "CBB applied to all rows.")
    return results
...
def _validate_arrays(values_lists, logger=None):
    t = values_lists[0]
    t_type = type(t)
